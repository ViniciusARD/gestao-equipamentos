...
export async function loadManageReservationsView(token, params = {}) {
    ...
    const sortOptions = [
        { key: '', text: 'Padrão (Pendentes Primeiro)' },
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-lg-6 col-md-12">
                        ...
                        <div class="btn-group w-100" role="group">
                            ${statusFilters.map(filter => `
                                <button type="button" class="btn ${(params.status || 'all') === filter.key ? 'btn-primary' : 'btn-outline-primary'} user-status-filter-btn" data-status="${filter.key}">${filter.text}</button>
                            `).join('')}
                        </div>
                    </div>
                     <div class="col-lg-4 col-md-6">
                        <label for="userSectorFilter" class="form-label small">Setor</label>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <label for="userSortBy" class="form-label small">Ordenar por</label>
                        <select id="userSortBy" class="form-select">
                             ${sortOptions.map(opt => `<option value="${opt.key}" ${params.sort_by === opt.key ? 'selected' : ''}>${opt.text}</option>`).join('')}
                        </select>
                    </div>
                    <div class="col-lg-2 col-md-6">
                        <label for="userSortDir" class="form-label small">Direção</label>
                        <select id="userSortDir" class="form-select">
                            <option value="asc" ${params.sort_dir === 'asc' ? 'selected' : ''}>Ascendente</option>
                            <option value="desc" ${params.sort_dir === 'desc' ? 'selected' : ''}>Descendente</option>
                        </select>
                    </div>
                    <div class="col-lg-2 col-md-6 d-flex align-items-end">
                        <button class="btn btn-primary w-100" id="applyUsersFilterBtn"><i class="bi bi-funnel-fill"></i> Aplicar</button>
                    </div>
                </div>
            </div>
        </div>
